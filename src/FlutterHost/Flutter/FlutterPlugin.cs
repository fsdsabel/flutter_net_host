...
namespace FlutterHost.Flutter
{
    public abstract class FlutterPlugin
    {
        ...
        {
            return FlutterWindowsInterop.FlutterDesktopMessengerSend(_messenger, 
                ChannelName, message, new IntPtr(message.Length));
        }
        ...
        protected Task<(bool success, byte[] reply)> SendMessageWithReplyAsync(byte[] message)
        {
            ...
            bool result = FlutterWindowsInterop.FlutterDesktopMessengerSendWithReply(_messenger,
                ChannelName, message, new IntPtr(message.Length),
                (data, size, _) =>
                {
                    tcs.SetResult((true, data));
                }, IntPtr.Zero);
        ...
        private void OnFlutterMessageReceived(IntPtr messenger, FlutterDesktopMessage message, IntPtr userData)
        {
            ...
            try
            {
                ...
                var result = methodInfo.Invoke(this,
                    MapFlutterParameters(methodInfo, methodCall.CastedArguments<IDictionary<object, object>>()));
            ...
            FlutterWindowsInterop.FlutterDesktopMessengerSendResponse(_messenger,
                    message.ResponseHandle, resultData, new IntPtr(resultData.Length));
        }
        ...
        protected virtual MethodInfo FindFlutterMethod(MethodCall methodCall)
        {
            return GetType().GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                .Where(m => m.GetCustomAttribute<FlutterMethodAttribute>()?.MethodName == methodCall.Method)
        ...
        protected virtual object[] MapFlutterParameters(MethodInfo targetMethod, IDictionary<object, object> arguments)
        {
            ...
            for(int i=0;i< parameters.Length;i++)
            {
                if (arguments.TryGetValue(parameters[i].Name, out var value))
                {
                    args[i] = value;
                }
            }
